...
| 缺少持续性能回归门禁 | 原有主要 workflow 偏向 tag/release；公开 benchmark 主要回答是否领先 SQLite/DuckDB | 第一阶段 | PR 和 nightly 必须比较 ApexBase base/current 自身基线 |
...
3. PR 执行一组稳定的 canary 指标。
4. 完整 78 项 benchmark 放在固定机器的 nightly 任务中。
...
- PR canary 实际接入 CI 并比较同机的 base/current commit。
- 完整 78 项 nightly 在明确的固定机器或等价的稳定性能环境运行。
...
> 本节已根据用户于 2026-07-19 的明确许可同步当前工作进度。后续如需再次更新，仍须重新取得用户明确许可。

### 第一阶段：进行中，不能标记为完成

已在当前工作区实现或验证：

- benchmark JSON 输出和微秒级精度，并记录 Git commit、branch、dirty 状态、系统、依赖和构建版本；完整元数据结构已有测试覆盖。
- ApexBase 自身 base/current 性能比较工具，以及缺失指标、配置不兼容和超阈值回退的失败测试。
- PR canary 和 nightly workflow 草案；PR canary 已按同一 runner 的 base/current commit 设计比较流程。
- 共享 SQL 路由语料已固化为独立、可维护的 `test/fixtures/query_signature_routes.jsonc`，19 类 SQL 同时经过 Python/Rust 分类器的一致性测试。
- 独立 cache invalidation 架构契约测试，覆盖跨客户端直接写、事务提交和 schema rewrite；测试发现并修复了事务提交后字典型 GROUP BY 读取陈旧数据的问题。
- 独立 delta、compact、reopen 行为契约测试。
- cache invalidation 步骤已完成一次修改前后完整验收：release 构建成功；完整 pytest 1461/1461 通过，合规采样为 8.91 秒；完整 cargo test 的 391 个单元测试和 6 个文档测试全部通过；修改后 72 项表格 benchmark 和 6 项向量 benchmark 均无实质性能回退。

尚未完成：

- workflow 尚未提交或实际接入 CI。
- nightly 使用 GitHub 托管 runner，尚不能等同于固定性能机器。
- pytest 已出现 8.91 秒和 9.41 秒两次相邻采样，尚未完成接近 9 秒的重复测量、耗时归因和波动范围分析。
- 尚未设计并接入可重复的 9 秒自动门禁，当前仍不能依靠单次合规采样认定该项完成。
- 第一阶段的全部完成标准尚未同时满足，因此第一阶段仍不能标记为完成。
...
4. [ ] 重复测量并归因 pytest 耗时和波动。
5. [ ] 设计可重复的 9 秒自动门禁。
6. [ ] 明确 nightly 的固定性能环境方案。
7. [ ] 完整执行 `AGENTS.md` 验收流程。
8. [ ] 向用户逐项报告第一阶段结果；只有满足全部完成标准后，才能请求进入第二阶段。
