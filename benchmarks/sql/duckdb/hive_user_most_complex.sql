...
p AS (
    SELECT
        ...
        hive_date_sub('${biz_date}', 1) AS d1,
        hive_date_sub('${biz_date}', 3) AS d3,
        hive_date_sub('${biz_date}', 7) AS d7,
        hive_date_sub('${biz_date}', 14) AS d14,
        hive_date_sub('${biz_date}', 30) AS d30,
        hive_date_sub('${biz_date}', 60) AS d60,
        hive_date_sub('${biz_date}', 90) AS d90,
        hive_date_sub('${biz_date}', 180) AS d180,
        hive_date_sub('${biz_date}', 365) AS d365,
        epoch(strptime('${biz_date}', '%Y-%m-%d')) AS biz_ts,
        'CN' AS country_code,
...
/* ============================================================
   ...
   ============================================================ */
life_cycle_rule AS (
    SELECT * FROM (VALUES ('NEW_1D', 0, 1, '当日新客'), ('NEW_7D', 2, 7, '7日新客'), ('GROWING_30D', 8, 30, '成长期用户'), ('ACTIVE_90D', 31, 90, '活跃用户'), ('MATURE_180D', 91, 180, '成熟用户'), ('OLD_365D', 181, 365, '老用户'), ('SUPER_OLD', 366, 99999, '超老用户'), ('SILENT', -1, -1, '沉默用户'), ('LOST', -2, -2, '流失用户')) AS stack_values(
        life_cycle_code,
        min_register_days,
        max_register_days,
        life_cycle_name
...
/* ============================================================
   ...
   ============================================================ */
risk_rule AS (
    SELECT * FROM (VALUES ('R0', 0, 9, '无风险'), ('R1', 10, 29, '低风险'), ('R2', 30, 49, '中风险'), ('R3', 50, 69, '高风险'), ('R4', 70, 89, '极高风险'), ('R5', 90, 100, '黑名单风险'), ('RX', -1, -1, '未知风险')) AS stack_values(
        risk_level,
        min_score,
        max_score,
        risk_name
...
/* ============================================================
   ...
   ============================================================ */
channel_rule AS (
    SELECT * FROM (VALUES ('APP_PUSH', 'PRIVATE', 0.18, 1.00), ('SMS', 'PRIVATE', 0.12, 0.90), ('EMAIL', 'PRIVATE', 0.08, 0.80), ('WECHAT', 'PRIVATE', 0.22, 1.10), ('DOUYIN', 'CONTENT', 0.35, 1.30), ('KUAISHOU', 'CONTENT', 0.30, 1.20), ('XIAOHONGSHU', 'CONTENT', 0.28, 1.15), ('BILIBILI', 'CONTENT', 0.20, 1.05), ('SEARCH_ENGINE', 'SEARCH', 0.26, 1.00), ('SEO', 'SEARCH', 0.10, 0.70), ('SEM', 'SEARCH', 0.32, 1.20), ('AFFILIATE', 'DIST', 0.20, 0.95), ('OFFLINE_STORE', 'OFFLINE', 0.40, 1.50), ('LIVE', 'LIVE', 0.38, 1.40), ('NATURAL', 'NATURAL', 0.05, 0.50), ('UNKNOWN', 'UNKNOWN', 0.01, 0.10)) AS stack_values(
        channel_code,
        channel_group,
        base_weight,
        quality_weight
...
/* ============================================================
   ...
   ============================================================ */
price_band_rule AS (
    SELECT * FROM (VALUES ('P00', 0.00, 9.99, '白菜价'), ('P01', 10.00, 19.99, '超低价'), ('P02', 20.00, 49.99, '低价'), ('P03', 50.00, 99.99, '平价'), ('P04', 100.00, 199.99, '中低价'), ('P05', 200.00, 499.99, '中价'), ('P06', 500.00, 999.99, '中高价'), ('P07', 1000.00, 2999.99, '高价'), ('P08', 3000.00, 9999.99, '高端'), ('P09', 10000.00, 999999999, '奢侈')) AS stack_values(
        price_band_code,
        min_price,
        max_price,
        price_band_name
...
/* ============================================================
   ...
   ============================================================ */
tag_rule AS (
    SELECT * FROM (VALUES ('T_SUPER_VALUE', '超级高价值', 'net_pay_amount_365d >= 5000'), ('T_HIGH_VALUE', '高价值', 'net_pay_amount_365d >= 1000'), ('T_LOW_VALUE', '低价值', 'net_pay_amount_365d < 100'), ('T_NEW_ACTIVE', '新客活跃', 'register_days <= 30 and event_cnt_7d > 0'), ('T_NEW_PAY', '新客成交', 'register_days <= 30 and paid_order_cnt_30d > 0'), ('T_SILENT', '沉默', 'days_since_last_event >= 30'), ('T_LOST', '流失', 'days_since_last_event >= 90'), ('T_RISK_HIGH', '高风险', 'risk_score >= 70'), ('T_BLACK', '黑名单', 'is_black_user = 1'), ('T_REFUND_HEAVY', '退款偏高', 'refund_rate_365d >= 0.35'), ('T_COUPON_SENSITIVE', '券敏感', 'coupon_use_rate_180d >= 0.6'), ('T_MARKETING_POSITIVE', '营销正反馈', 'marketing_roi_90d >= 0.8'), ('T_LIVE_USER', '直播用户', 'live_enter_cnt_90d > 0'), ('T_SEARCH_HEAVY', '搜索重度', 'search_cnt_90d >= 30'), ('T_CART_LOST', '加购流失', 'add_cart_cnt_90d > pay_success_cnt_90d'), ('T_MULTI_DEVICE', '多设备', 'device_cnt_90d >= 3'), ('T_MULTI_IP', '多IP', 'ip_cnt_90d >= 5'), ('T_OFFLINE_USER', '线下用户', 'store_visit_cnt_90d > 0'), ('T_COMPLAINT_HIGH', '高投诉', 'complaint_ticket_cnt_180d >= 3'), ('T_FAST_PAY', '极速支付', 'avg_order_to_pay_sec_365d <= 5'), ('T_DELAY_SENSITIVE', '履约敏感', 'timeout_fulfillment_rate_365d >= 0.2'), ('T_HIGH_MARGIN', '高毛利', 'gross_margin_rate_365d >= 0.35'), ('T_PRICE_SENSITIVE', '价格敏感', 'discount_rate_365d >= 0.3'), ('T_EXPERIMENT_USER', '实验用户', 'ab_exp_cnt_90d > 0')) AS stack_values(
        tag_code,
        tag_name,
        tag_expr
    )
...
user_base AS (
    SELECT
        *,
        ...
        CASE
            WHEN regexp_matches(upper(register_channel), 'DOUYIN|KUAISHOU|XIAOHONGSHU|BILIBILI|LIVE') THEN 'CONTENT'
            WHEN regexp_matches(upper(register_channel), 'SMS|PUSH|EMAIL|WECHAT') THEN 'PRIVATE'
            WHEN regexp_matches(upper(register_channel), 'STORE|OFFLINE') THEN 'OFFLINE'
            WHEN regexp_matches(upper(register_channel), 'SEO|SEM|SEARCH') THEN 'SEARCH'
            WHEN regexp_matches(upper(register_channel), 'NATURAL|ORGANIC') THEN 'NATURAL'
            ELSE 'OTHER'
...
user_preference_kv AS (
    SELECT user_id, list_extract(string_split(pref_pair, ':'), 1) AS pref_key, list_extract(string_split(pref_pair, ':'), 2) AS pref_value FROM user_base
    ...
    ) pref(pref_pair) ON TRUE
    
    WHERE coalesce(list_extract(string_split(pref_pair, ':'), 1), '') <> ''
),

user_preference_map AS (
    SELECT
        user_id,
        array_to_string(collect_list(concat(pref_key, ':', pref_value)), ',') AS preference_map,
        array_to_string(sort_array(collect_set(pref_key)), ',') AS preference_key_list
...
event_exposure_long AS (
    SELECT
        ...
        expose_sku_id
    FROM event_raw er
    LEFT JOIN LATERAL (SELECT item_pos - 1 AS expose_pos, list_extract(er.expose_sku_arr, item_pos) AS expose_sku_id FROM UNNEST(range(1, len(er.expose_sku_arr) + 1)) u(item_pos)) px ON TRUE
...
virtual_session_agg AS (
    SELECT
        ...
        array_to_string(collect_list(event_type), '>') AS virtual_session_event_path
    FROM (
        SELECT *
        FROM session_event_ordered
        ORDER BY user_id, session_id, virtual_session_seq, event_ts, event_id
    ) t
...
event_user_agg AS (
    SELECT
        ...
        sum(if(er.dt >= hive_date_sub('${biz_date}', 30), 1, 0)) AS event_cnt_30d,
        sum(if(er.dt >= hive_date_sub('${biz_date}', 7), 1, 0)) AS event_cnt_7d,
        sum(if(er.dt >= hive_date_sub('${biz_date}', 1), 1, 0)) AS event_cnt_1d,

        count(DISTINCT er.session_id) AS session_cnt_90d,
        count(DISTINCT if(er.dt >= hive_date_sub('${biz_date}', 30), er.session_id, NULL)) AS session_cnt_30d,
        count(DISTINCT if(er.dt >= hive_date_sub('${biz_date}', 7), er.session_id, NULL)) AS session_cnt_7d,
        ...
        array_to_string(sort_array(collect_set(er.channel)), ',') AS channel_set_90d,
        array_to_string(sort_array(collect_set(er.sub_channel)), ',') AS sub_channel_set_90d,
        array_to_string(sort_array(collect_set(er.device_type)), ',') AS device_type_set_90d,
        array_to_string(sort_array(collect_set(er.os)), ',') AS os_set_90d
...
virtual_session_user_agg AS (
    SELECT
        ...
        quantile_cont(virtual_session_event_cnt, 0.5) AS median_virtual_session_event_cnt_90d,
        ...
        quantile_cont(virtual_session_duration_sec, 0.5) AS median_virtual_session_duration_sec_90d,
        max(virtual_session_duration_sec) AS max_virtual_session_duration_sec_90d,
        array_to_string(list_slice(list_reverse(
                    sort_array(
                        collect_list(
                            concat(
                ...
                ), 1, (1) + (10) - 1), '||') AS top10_virtual_session_path_90d
...
exposure_user_agg AS (
    SELECT
        ...
        quantile_cont(eel.expose_pos, 0.5) AS median_exposure_pos_90d,
        array_to_string(sort_array(collect_set(sd.category_level1_name)), ',') AS exposure_cate1_name_set_90d,
        array_to_string(sort_array(collect_set(sd.price_band_code)), ',') AS exposure_price_band_set_90d
...
trade_user_agg AS (
    SELECT
        ...
        count(DISTINCT if(dt >= hive_date_sub('${biz_date}', 180), order_id, NULL)) AS order_cnt_180d,
        count(DISTINCT if(dt >= hive_date_sub('${biz_date}', 90), order_id, NULL)) AS order_cnt_90d,
        count(DISTINCT if(dt >= hive_date_sub('${biz_date}', 30), order_id, NULL)) AS order_cnt_30d,
        count(DISTINCT if(dt >= hive_date_sub('${biz_date}', 7), order_id, NULL)) AS order_cnt_7d,

        count(DISTINCT if(pay_status = 'PAID', order_id, NULL)) AS paid_order_cnt_365d,
        count(DISTINCT if(pay_status = 'PAID' AND dt >= hive_date_sub('${biz_date}', 180), order_id, NULL)) AS paid_order_cnt_180d,
        count(DISTINCT if(pay_status = 'PAID' AND dt >= hive_date_sub('${biz_date}', 90), order_id, NULL)) AS paid_order_cnt_90d,
        count(DISTINCT if(pay_status = 'PAID' AND dt >= hive_date_sub('${biz_date}', 30), order_id, NULL)) AS paid_order_cnt_30d,

        count(DISTINCT if(refund_id IS NOT NULL, order_id, NULL)) AS refund_order_cnt_365d,
        count(DISTINCT if(refund_id IS NOT NULL AND dt >= hive_date_sub('${biz_date}', 90), order_id, NULL)) AS refund_order_cnt_90d,

        sum(if(pay_status = 'PAID', pay_amount, 0)) AS gross_pay_amount_365d,
        sum(if(pay_status = 'PAID' AND dt >= hive_date_sub('${biz_date}', 180), pay_amount, 0)) AS gross_pay_amount_180d,
        sum(if(pay_status = 'PAID' AND dt >= hive_date_sub('${biz_date}', 90), pay_amount, 0)) AS gross_pay_amount_90d,
        sum(if(pay_status = 'PAID' AND dt >= hive_date_sub('${biz_date}', 30), pay_amount, 0)) AS gross_pay_amount_30d,

        sum(net_pay_amount) AS net_pay_amount_365d,
        sum(if(dt >= hive_date_sub('${biz_date}', 180), net_pay_amount, 0)) AS net_pay_amount_180d,
        sum(if(dt >= hive_date_sub('${biz_date}', 90), net_pay_amount, 0)) AS net_pay_amount_90d,
        sum(if(dt >= hive_date_sub('${biz_date}', 30), net_pay_amount, 0)) AS net_pay_amount_30d,

        sum(gross_profit_amount) AS gross_profit_amount_365d,
        sum(if(dt >= hive_date_sub('${biz_date}', 90), gross_profit_amount, 0)) AS gross_profit_amount_90d,
        ...
        quantile_cont(order_to_pay_sec, 0.5) AS median_order_to_pay_sec_365d,
        avg(pay_to_sign_sec) AS avg_pay_to_sign_sec_365d,
        quantile_cont(pay_to_sign_sec, 0.5) AS median_pay_to_sign_sec_365d,
        ...
        array_to_string(sort_array(collect_set(payment_channel)), ',') AS payment_channel_set_365d,
        array_to_string(sort_array(collect_set(payment_method)), ',') AS payment_method_set_365d,
        array_to_string(sort_array(collect_set(delivery_type)), ',') AS delivery_type_set_365d,
        array_to_string(sort_array(collect_set(price_band_code)), ',') AS bought_price_band_set_365d
...
marketing_touch_raw AS (
    ...
    WHERE dt BETWEEN hive_date_sub('${biz_date}', 90) AND '${biz_date}'
...
marketing_user_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(channel)), ',') AS marketing_channel_set_90d,
        array_to_string(sort_array(collect_set(channel_group)), ',') AS marketing_channel_group_set_90d,
        ...
        quantile_cont(touch_to_pay_sec, 0.5) AS median_touch_to_pay_sec_90d,
...
coupon_raw AS (
    ...
    WHERE c.dt BETWEEN hive_date_sub('${biz_date}', 180) AND '${biz_date}'
...
service_raw AS (
    ...
    WHERE s.dt BETWEEN hive_date_sub('${biz_date}', 180) AND '${biz_date}'
...
service_user_agg AS (
    SELECT
        ...
        quantile_cont(satisfaction_score, 0.5) AS median_service_satisfaction_score_180d,
        avg(unix_timestamp(solve_time) - unix_timestamp(create_time)) AS avg_ticket_solve_sec_180d,
        quantile_cont(unix_timestamp(solve_time) - unix_timestamp(create_time), 0.5) AS median_ticket_solve_sec_180d,
        max(create_time) AS last_service_create_time,
        array_to_string(sort_array(collect_set(ticket_type)), ',') AS service_ticket_type_set_180d
    FROM service_raw
    GROUP BY user_id
),
...
device_graph AS (
    SELECT
        ...
        array_to_string(list_slice(sort_array(collect_set(user_id)), 1, (1) + (30) - 1), ',') AS device_user_sample
    FROM event_raw
...
ip_graph AS (
    SELECT
        ...
        array_to_string(list_slice(sort_array(collect_set(user_id)), 1, (1) + (30) - 1), ',') AS ip_user_sample
    FROM event_raw
...
address_graph AS (
    SELECT
        ...
        array_to_string(list_slice(sort_array(collect_set(user_id)), 1, (1) + (30) - 1), ',') AS address_user_sample
    FROM order_item_enriched
...
user_event_graph_risk AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(dg.device_user_sample)), ',') AS risky_device_user_sample,
        array_to_string(sort_array(collect_set(ig.ip_user_sample)), ',') AS risky_ip_user_sample
    FROM event_raw er
...
user_address_graph_risk AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(ag.address_user_sample)), ',') AS risky_address_user_sample
...
risk_score_calc AS (
    SELECT
        ...
        regexp_replace(
            regexp_replace(
                array_to_string(list_value(if(coalesce(ub.is_black_user, 0) = 1, 'BLACK_USER', NULL), if(coalesce(eua.device_cnt_90d, 0) >= 10, 'MANY_DEVICE', NULL), if(coalesce(eua.ip_cnt_90d, 0) >= 20, 'MANY_IP', NULL), if(coalesce(ugr.max_device_bind_user_cnt_90d, 0) >= 5, 'DEVICE_GRAPH_RISK', NULL), if(coalesce(ugr.max_ip_bind_user_cnt_90d, 0) >= 10, 'IP_GRAPH_RISK', NULL), if(coalesce(ugr.max_address_bind_user_cnt_365d, 0) >= 5, 'ADDRESS_GRAPH_RISK', NULL), if(coalesce(tua.refund_rate_365d, 0) >= 0.35, 'HIGH_REFUND_RATE', NULL), if(coalesce(tua.refund_amount_rate_365d, 0) >= 0.50, 'HIGH_REFUND_AMOUNT_RATE', NULL), if(ub.email_domain IN ('tempmail.com', 'mailinator.com', 'fake.com', 'trashmail.com'), 'TEMP_EMAIL', NULL)), ','),
                ',+',
                ','
            ),
            '^,|,$',
...
user_metric_long AS (
    ...
    CROSS JOIN LATERAL (VALUES ('behavior', 'event_cnt_90d', cast(event_cnt_90d AS double)), ('behavior', 'session_cnt_90d', cast(session_cnt_90d AS double)), ('behavior', 'expose_click_rate_90d', cast(expose_click_rate_90d AS double)), ('behavior', 'click_cart_rate_90d', cast(click_cart_rate_90d AS double)), ('behavior', 'submit_pay_rate_90d', cast(submit_pay_rate_90d AS double)), ('trade', 'order_cnt_365d', cast(order_cnt_365d AS double)), ('trade', 'paid_order_cnt_365d', cast(paid_order_cnt_365d AS double)), ('trade', 'net_pay_amount_365d', cast(net_pay_amount_365d AS double)), ('trade', 'gross_margin_rate_365d', cast(gross_margin_rate_365d AS double)), ('trade', 'refund_rate_365d', cast(refund_rate_365d AS double)), ('coupon', 'coupon_use_rate_180d', cast(coupon_use_rate_180d AS double)), ('marketing', 'marketing_roi_90d', cast(marketing_roi_90d AS double)), ('risk', 'risk_score', cast(risk_score AS double))) s(metric_group, metric_name, metric_value)
),
...
user_tag_agg AS (
    SELECT
        user_id,
        array_to_string(sort_array(collect_set(tag_code)), ',') AS tag_code_list,
        array_to_string(sort_array(collect_set(tag_name)), ',') AS tag_name_list,
...
active_no_pay_user AS (
    ...
    ANTI JOIN (
        ...
        FROM order_item_enriched
        WHERE pay_status = 'PAID'
          AND dt >= hive_date_sub('${biz_date}', 90)
    ) paid
...
category_channel_cube AS (
    ...
    FROM order_item_enriched
    WHERE pay_status = 'PAID'
    GROUP BY CUBE (category_level1_name,
        price_band_name,
        payment_channel,
        delivery_type)
),
...
final_user_extreme_wide AS (
    SELECT
        ...
        md5(
            concat_ws('|', ub.user_id, coalesce(lcf.final_life_cycle_code, ''), coalesce(cast(eua.event_cnt_90d AS string), '0'), coalesce(cast(tua.net_pay_amount_365d AS string), '0'), coalesce(cast(rua.risk_score AS string), '0'), coalesce(uta.tag_code_list, ''))
...
CREATE OR REPLACE TABLE ads_user.ads_user_extreme_operation_profile_df AS SELECT branch.*, '${biz_date}' AS dt, 'NEW_RETAIL_ECOM' AS biz_line FROM (SELECT * FROM __hive_multi_source f WHERE f.user_id IS NOT NULL
  AND coalesce(f.is_employee, 0) = 0) branch;
CREATE OR REPLACE TABLE ads_user.ads_high_value_user_pool_df AS SELECT branch.*, '${biz_date}' AS dt FROM (SELECT f.user_id,
    f.mobile,
    f.email,
    f.province,
    f.city,
    ...
    f.tag_code_list,
    f.user_feature_hash,
    current_timestamp AS etl_time FROM __hive_multi_source f WHERE f.net_pay_amount_365d >= 1000
  AND f.risk_score < 70) branch;
CREATE OR REPLACE TABLE ads_user.ads_user_recall_pool_df AS SELECT branch.*, '${biz_date}' AS dt FROM (SELECT f.user_id,
    f.mobile,
    f.email,
    f.province,
    f.city,
    ...
    current_timestamp AS etl_time FROM __hive_multi_source f WHERE f.risk_score < 70
  AND (
