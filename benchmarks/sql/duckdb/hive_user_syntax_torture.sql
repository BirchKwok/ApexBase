...
channel_rule AS (
    SELECT * FROM (VALUES ('APP_PUSH', 0.18, 'PRIVATE'), ('EMAIL', 0.08, 'PRIVATE'), ('DOUYIN', 0.35, 'PAID_MEDIA'), ('WECHAT', 0.22, 'PRIVATE'), ('OFFLINE_STORE', 0.40, 'OFFLINE'), ('NATURAL', 0.05, 'ORGANIC'), ('UNKNOWN', 0.01, 'UNKNOWN')) AS stack_values(channel, channel_weight, channel_group)
),

risk_rule AS (
    SELECT * FROM (VALUES ('R0', 0, 19, 'LOW'), ('R1', 20, 39, 'MEDIUM_LOW'), ('R2', 40, 59, 'MEDIUM'), ('R3', 60, 79, 'HIGH'), ('R4', 80, 100, 'EXTREME')) AS stack_values(risk_code, score_min, score_max, risk_desc)
),
...
user_base AS (
    SELECT
        ...
        CASE
            WHEN regexp_matches(upper(u.register_channel), 'DOUYIN|KUAISHOU|XIAOHONGSHU') THEN 'CONTENT_MEDIA'
            WHEN regexp_matches(upper(u.register_channel), 'EMAIL|SMS|PUSH|WECHAT') THEN 'PRIVATE_DOMAIN'
            WHEN regexp_matches(upper(u.register_channel), 'STORE|OFFLINE') THEN 'OFFLINE'
            ELSE 'OTHER'
...
user_preference_long AS (
    SELECT
        ub.user_id,
        pref_pos,
        list_extract(string_split(pref_pair, ':'), 1) AS pref_key,
        list_extract(string_split(pref_pair, ':'), 2) AS pref_value
    FROM user_base ub
    LEFT JOIN LATERAL (SELECT item_pos - 1 AS pref_pos, list_extract(
        split(coalesce(get_json_object(
            concat('{"profile":', coalesce('{}', '{}'), '}'),
            '$.profile.preference_json'
        ), 'color:red,size:m'), ',')
    , item_pos) AS pref_pair FROM UNNEST(range(1, len(
        split(coalesce(get_json_object(
            concat('{"profile":', coalesce('{}', '{}'), '}'),
            '$.profile.preference_json'
        ), 'color:red,size:m'), ',')
    ) + 1)) u(item_pos)) p ON TRUE
),
...
event_exploded AS (
    ...
    FROM event_raw er
    LEFT JOIN LATERAL (SELECT item_pos - 1 AS expose_pos, list_extract(er.expose_sku_arr, item_pos) AS expose_sku_id FROM UNNEST(range(1, len(er.expose_sku_arr) + 1)) u(item_pos)) ex ON TRUE
),
...
event_user_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(channel)), ',') AS channel_path_90d,
        array_to_string(collect_list(event_type), '>') AS raw_event_path_90d,
        quantile_cont(event_ts, 0.25) AS event_ts_p25,
        quantile_cont(event_ts, 0.50) AS event_ts_p50,
        quantile_cont(event_ts, 0.75) AS event_ts_p75,
...
trade_user_agg AS (
    SELECT
        ...
        quantile_cont(pay_amount, 0.50) AS median_item_pay_amount_365d,
...
refund_user_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(r.refund_reason_code)), ',') AS refund_reason_set_365d
...
marketing_user_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(touch_channel)), ',') AS marketing_channel_set_90d,
        array_to_string(sort_array(collect_set(channel_group)), ',') AS marketing_group_set_90d,
        quantile_cont(touch_ts, 0.50) AS median_touch_ts_90d
...
service_user_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(s.ticket_type)), ',') AS ticket_type_set_365d
...
metric_long AS (
    ...
    CROSS JOIN LATERAL (VALUES ('event_cnt_90d', cast(etf.event_cnt_90d AS double)), ('paid_order_cnt_365d', cast(etf.paid_order_cnt_365d AS double)), ('pay_amount_365d', cast(etf.pay_amount_365d AS double))) m(metric_name, metric_value)
),

metric_map AS (
    SELECT
        user_id,
        array_to_string(sort_array(collect_set(
...
channel_member_cube AS (
    SELECT
        ...
        count(DISTINCT ub.user_id) AS user_cnt
    FROM user_base ub
    GROUP BY CUBE (ub.register_channel_group, ub.member_level)
),
...
province_city_rollup AS (
    SELECT
        ...
        count(DISTINCT ub.user_id) AS user_cnt
    FROM user_base ub
    GROUP BY ROLLUP (ub.province, ub.city)
),
...
final_user_extreme_wide AS (
    SELECT
        ...
        CASE
            ...
            ELSE 'SILENT'
        END AS user_segment,
        md5(concat_ws('|', ru.user_id, cast(ru.event_cnt_90d AS string), cast(ru.pay_amount_365d AS string), cast(ru.risk_score AS string), coalesce(ru.metric_map_365d, ''))) AS feature_hash,
...
CREATE OR REPLACE TABLE ads_user.ads_user_syntax_torture_profile_df AS SELECT branch.*, '${biz_date}' AS dt, 'OMNICHANNEL' AS biz_line FROM (SELECT * FROM __hive_multi_source f WHERE f.user_id IS NOT NULL
  AND coalesce(f.is_employee, 0) = 0) branch;
CREATE OR REPLACE TABLE ads_user.ads_user_syntax_torture_alert_df AS SELECT branch.*, '${biz_date}' AS dt FROM (SELECT f.user_id,
    f.mobile,
    f.email,
    f.province,
    f.city,
    ...
    f.feature_hash,
    current_timestamp AS etl_time FROM __hive_multi_source f WHERE coalesce(f.is_employee, 0) = 0
  AND (
      f.risk_score >= 60
