CREATE OR REPLACE TABLE ads_user.ads_user_super_operation_profile_df AS
SELECT source_result.*, '${biz_date}' AS dt, 'NEW_RETAIL' AS biz_line FROM (
...
/* ============================================================
   ...
   ============================================================ */
life_cycle_rule AS (
    SELECT * FROM (VALUES ('NEW_USER', 0, 7, '新用户'), ('GROWING_USER', 8, 30, '成长期用户'), ('ACTIVE_USER', 31, 90, '活跃用户'), ('MATURE_USER', 91, 180, '成熟用户'), ('OLD_USER', 181, 9999, '老用户'), ('SILENT_USER', -1, -1, '沉默用户'), ('LOST_USER', -2, -2, '流失用户'), ('UNKNOWN_USER', -9, -9, '未知用户')) AS stack_values(
        life_cycle_code,
        min_days,
        max_days,
        life_cycle_name
...
/* ============================================================
   ...
   ============================================================ */
risk_score_rule AS (
    SELECT * FROM (VALUES ('R0', 0, 10, '极低风险'), ('R1', 11, 30, '低风险'), ('R2', 31, 50, '中风险'), ('R3', 51, 70, '高风险'), ('R4', 71, 90, '极高风险'), ('R5', 91, 100, '黑名单风险')) AS stack_values(
        risk_level,
        min_score,
        max_score,
        risk_desc
...
channel_weight_rule AS (
    SELECT * FROM (VALUES ('APP_PUSH', 0.18, '私域触达'), ('SMS', 0.12, '私域触达'), ('EMAIL', 0.08, '私域触达'), ('WECHAT', 0.22, '私域触达'), ('DOUYIN', 0.35, '外部投放'), ('KUAISHOU', 0.30, '外部投放'), ('XIAOHONGSHU', 0.28, '内容种草'), ('SEARCH_ENGINE', 0.26, '搜索广告'), ('AFFILIATE', 0.20, '联盟分销'), ('OFFLINE_STORE', 0.40, '线下门店'), ('NATURAL', 0.05, '自然流量'), ('UNKNOWN', 0.01, '未知渠道')) AS stack_values(
        channel,
        base_weight,
        channel_group
    )
...
/* ============================================================
   ...
   ============================================================ */
price_band_rule AS (
    SELECT * FROM (VALUES ('P0', 0, 19.99, '超低价'), ('P1', 20, 49.99, '低价'), ('P2', 50, 99.99, '平价'), ('P3', 100, 299.99, '中价'), ('P4', 300, 999.99, '高价'), ('P5', 1000, 4999.99, '高端'), ('P6', 5000, 99999999, '奢侈')) AS stack_values(
        price_band_code,
        min_price,
        max_price,
        price_band_name
...
user_base AS (
    SELECT
        ...
        d365,
        CASE
            WHEN regexp_matches(register_channel, 'douyin|kuaishou|xiaohongshu') THEN 'CONTENT_MEDIA'
            WHEN regexp_matches(register_channel, 'sms|push|wechat|email') THEN 'PRIVATE_DOMAIN'
            WHEN regexp_matches(register_channel, 'store|offline') THEN 'OFFLINE'
            WHEN regexp_matches(register_channel, 'search|sem|seo') THEN 'SEARCH'
            ELSE 'OTHER'
...
event_expose_sku AS (
    SELECT
        ...
        expose_sku_id
    FROM event_raw er
    LEFT JOIN LATERAL (SELECT item_pos - 1 AS expose_pos, list_extract(er.expose_sku_arr, item_pos) AS expose_sku_id FROM UNNEST(range(1, len(er.expose_sku_arr) + 1)) u(item_pos)) pe ON TRUE
...
session_path_agg AS (
    SELECT
        user_id,
        session_id,
        array_to_string(collect_list(event_type), '>') AS session_event_path,
    ...
    FROM (
        SELECT *
        FROM event_raw
        ORDER BY user_id, session_id, event_ts, event_id
    ) x
...
session_user_agg AS (
    SELECT
        ...
        quantile_cont(session_event_cnt, 0.5) AS median_session_event_cnt_90d,
        ...
        quantile_cont(session_duration_sec, 0.5) AS median_session_duration_sec_90d,
        ...
        array_to_string(list_slice(list_reverse(
                ...
                ), 1, (1) + (10) - 1), '||') AS top10_complex_session_paths_90d
...
exposure_sku_agg AS (
    SELECT
        ...
        quantile_cont(ees.expose_pos, 0.5) AS median_exposure_pos_90d,
...
search_keyword_agg AS (
    SELECT
        ...
        array_to_string(list_slice(list_reverse(
                ...
                ), 1, (1) + (20) - 1), ',') AS top20_search_keywords_90d
    FROM (
        SELECT
            user_id,
            clean_keyword,
...
trade_user_agg AS (
    SELECT
        ...
        sum(if(pay_status = 'PAID' AND dt >= hive_date_sub('${biz_date}', 7), pay_amount, 0)) AS gross_pay_amount_7d,
        ...
        sum(if(dt >= hive_date_sub('${biz_date}', 90), quantity, 0)) AS item_quantity_90d,
...
device_user_graph AS (
    SELECT
        ...
        array_to_string(list_slice(sort_array(collect_set(user_id)), 1, (1) + (20) - 1), ',') AS device_sample_user_list
    FROM event_raw
...
ip_user_graph AS (
    SELECT
        ...
        array_to_string(list_slice(sort_array(collect_set(user_id)), 1, (1) + (20) - 1), ',') AS ip_sample_user_list
    FROM event_raw
...
address_user_graph AS (
    SELECT
        ...
        array_to_string(list_slice(sort_array(collect_set(user_id)), 1, (1) + (20) - 1), ',') AS address_sample_user_list
    FROM order_item_enriched
...
user_event_graph_risk AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(dug.device_sample_user_list)), ',') AS risky_device_related_users,
        array_to_string(sort_array(collect_set(iug.ip_sample_user_list)), ',') AS risky_ip_related_users
    FROM event_raw er
...
user_address_graph_risk AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(aug.address_sample_user_list)), ',') AS risky_address_related_users
...
risk_score_calc AS (
    SELECT
        ...
        array_to_string(list_value(if(coalesce(ub.is_black_user, 0) = 1, 'BLACK_USER', NULL), if(coalesce(eua.device_cnt_90d, 0) >= 10, 'MANY_DEVICES', NULL), if(coalesce(eua.ip_cnt_90d, 0) >= 20, 'MANY_IPS', NULL), if(coalesce(ugr.max_device_bind_user_cnt_90d, 0) >= 5, 'DEVICE_GRAPH_RISK', NULL), if(coalesce(ugr.max_ip_user_cnt_90d, 0) >= 10, 'IP_GRAPH_RISK', NULL), if(coalesce(ugr.max_address_user_cnt_365d, 0) >= 5, 'ADDRESS_GRAPH_RISK', NULL), if(coalesce(tua.refund_order_rate_365d, 0) >= 0.35, 'HIGH_REFUND_RATE', NULL), if(coalesce(tua.refund_amount_rate_365d, 0) >= 0.50, 'HIGH_REFUND_AMOUNT_RATE', NULL), if(ub.email_domain IN ('tempmail.com', 'mailinator.com', 'fake.com', 'trashmail.com'), 'TEMP_EMAIL', NULL)), ',') AS risk_reason_raw
...
user_metric_long AS (
    ...
    CROSS JOIN LATERAL (VALUES ('behavior', 'event_cnt_90d', cast(event_cnt_90d AS double)), ('behavior', 'session_cnt_90d', cast(session_cnt_90d AS double)), ('behavior', 'expose_click_rate_90d', cast(expose_click_rate_90d AS double)), ('behavior', 'click_cart_rate_90d', cast(click_cart_rate_90d AS double)), ('behavior', 'submit_pay_rate_90d', cast(submit_pay_rate_90d AS double)), ('trade', 'order_cnt_365d', cast(order_cnt_365d AS double)), ('trade', 'paid_order_cnt_365d', cast(paid_order_cnt_365d AS double)), ('trade', 'gross_pay_amount_365d', cast(gross_pay_amount_365d AS double)), ('trade', 'net_pay_amount_365d', cast(net_pay_amount_365d AS double)), ('trade', 'refund_order_rate_365d', cast(refund_order_rate_365d AS double)), ('coupon', 'coupon_use_rate_180d', cast(coupon_use_rate_180d AS double)), ('marketing', 'marketing_roi_90d', cast(marketing_roi_90d AS double)), ('risk', 'risk_score', cast(risk_score AS double))) s(metric_group, metric_name, metric_value)
),
...
/* ============================================================
   ...
   ============================================================ */
tag_rule AS (
    SELECT * FROM (VALUES ('SUPER_VALUE_USER', '超级高价值用户', 'net_pay_amount_365d >= 5000'), ('HIGH_VALUE_USER', '高价值用户', 'net_pay_amount_365d >= 1000'), ('NEW_ACTIVE_USER', '新晋活跃用户', 'register_days <= 30 and event_cnt_7d > 0'), ('SILENT_USER', '沉默用户', 'days_since_last_event >= 30'), ('LOST_USER', '流失用户', 'days_since_last_event >= 90'), ('HIGH_RISK_USER', '高风险用户', 'risk_score >= 70'), ('REFUND_HEAVY_USER', '退款偏高用户', 'refund_order_rate_365d >= 0.35'), ('COUPON_SENSITIVE_USER', '优惠券敏感用户', 'coupon_use_rate_180d >= 0.6'), ('CONTENT_MEDIA_USER', '内容渠道用户', 'register_channel_group = CONTENT_MEDIA'), ('OFFLINE_STORE_USER', '线下门店用户', 'store_visit_cnt_90d > 0'), ('LIVE_USER', '直播用户', 'live_enter_cnt_90d > 0'), ('SEARCH_HEAVY_USER', '搜索重度用户', 'search_cnt_90d >= 20'), ('CART_LOST_USER', '加购流失用户', 'add_cart_cnt_90d > pay_success_event_cnt_90d'), ('MULTI_DEVICE_USER', '多设备用户', 'device_cnt_90d >= 3'), ('HIGH_COMPLAINT_USER', '高投诉用户', 'complaint_ticket_cnt_180d >= 3'), ('MARKETING_POSITIVE_USER', '营销正反馈用户', 'marketing_roi_90d >= 1')) AS stack_values(
        tag_code,
        tag_name,
        tag_expr
    )
...
category_channel_cube AS (
    ...
    GROUP BY CUBE (oie.category_level1_name,
        oie.price_band_name,
        oie.payment_channel,
        oie.delivery_type)
),
...
final_user_super_wide AS (
    SELECT
        ...
        md5(
            concat_ws('|', ub.user_id, coalesce(ulcf.final_life_cycle_code, ''), coalesce(cast(tua.net_pay_amount_365d AS string), '0'), coalesce(cast(eua.event_cnt_90d AS string), '0'), coalesce(cast(rua.risk_score AS string), '0'), coalesce(uta.tag_code_list, ''))
