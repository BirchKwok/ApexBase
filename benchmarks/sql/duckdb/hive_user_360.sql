...
SELECT source_result.*, '${biz_date}' AS dt FROM (
WITH
/* ============================================================
...
/* ============================================================
   ...
   ============================================================ */
risk_level_map AS (
    SELECT * FROM (VALUES ('LOW', 0, 20, '低风险'), ('MEDIUM', 21, 50, '中风险'), ('HIGH', 51, 80, '高风险'), ('EXTREME', 81, 100, '极高风险'), ('UNKNOWN', -1, -1, '未知')) AS stack_values(risk_code, score_min, score_max, risk_desc)
),
...
behavior_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(be.channel)), ',') AS channel_path_30d,
        ...
        quantile_cont(unix_timestamp(be.event_time), 0.5) AS median_event_ts_30d,

        max(if(be.dt >= hive_date_sub('${biz_date}', 7), 1, 0)) AS is_active_7d,
        max(if(be.dt >= hive_date_sub('${biz_date}', 1), 1, 0)) AS is_active_1d
...
session_path AS (
    SELECT
        user_id,
        session_id,
        array_to_string(collect_list(event_type), '>') AS raw_event_path,
...
session_agg AS (
    SELECT
        ...
        quantile_cont(session_duration_sec, 0.5) AS median_session_duration_sec_30d,
        max(session_duration_sec) AS max_session_duration_sec_30d,
        array_to_string(list_slice(sort_array(
                    collect_list(
                        concat(
...
order_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(payment_method)), ',') AS payment_method_set_90d,
...
marketing_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(touch_channel)), ',') AS marketing_channel_set_30d,
...
order_metric_long AS (
    SELECT
        ...
        metric_value
    FROM order_agg
    CROSS JOIN LATERAL (VALUES ('order_cnt_90d', cast(order_cnt_90d AS double)), ('paid_order_cnt_90d', cast(paid_order_cnt_90d AS double)), ('refund_order_cnt_90d', cast(refund_order_cnt_90d AS double)), ('total_pay_amount_90d', cast(total_pay_amount_90d AS double)), ('avg_paid_order_amount_90d', cast(avg_paid_order_amount_90d AS double)), ('bought_category_cnt_90d', cast(bought_category_cnt_90d AS double)), ('bought_brand_cnt_90d', cast(bought_brand_cnt_90d AS double)), ('refund_order_rate_90d', cast(refund_order_rate_90d AS double))) s(metric_name, metric_value)
),
...
/* ============================================================
   ...
   ============================================================ */
tag_rule AS (
    SELECT * FROM (VALUES ('HIGH_VALUE_USER', '高价值用户', 'total_pay_amount_90d >= 1000'), ('NEW_ACTIVE_USER', '新晋活跃用户', 'is_new_user = 1 and is_active_7d = 1'), ('HIGH_RISK_USER', '高风险用户', 'risk_score >= 70'), ('COUPON_SENSITIVE', '优惠券敏感', 'coupon_use_rate_30d >= 0.5'), ('SEARCH_HEAVY_USER', '搜索重度用户', 'search_cnt_30d >= 20'), ('CART_LOST_USER', '加购未支付用户', 'add_cart_cnt_30d > pay_success_cnt_30d'), ('MULTI_DEVICE_USER', '多设备用户', 'device_cnt_30d >= 3'), ('REFUND_HEAVY_USER', '退款偏高用户', 'refund_order_rate_90d >= 0.3')) AS stack_values(tag_code, tag_name, tag_expr)
),
...
service_agg AS (
    SELECT
        ...
        array_to_string(sort_array(collect_set(s.ticket_type)), ',') AS service_type_set_90d
