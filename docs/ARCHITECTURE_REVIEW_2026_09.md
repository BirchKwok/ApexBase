...
完成；§14.5 余项仅剩 morsel 并行 B 期实现（存储层并行扫描 + 成本
自动启用，余项 3）。
...
3. **morsel 并行**（A 期已完成，见 §14.8/§14.11）：`APEX_PARALLEL_SCAN=N`
   ...
   小幅为负，自动启用决策须计入并发度）；端到端加速两规模最高
   1.112x、有效 worker 在预算上限 4 处饱和。余项：存储层并行扫描
   设计/评估（IO 密集段 worker 预算策略独立论证）+ B 期成本自动
   启用（依赖并行扫描独立成本类记录，机制复用 R5.3/R5.8）；并行段
   目前仅覆盖聚合阶段，扫描/物化段仍串行（§14.11.5-1，现以实测
   曲线背书，§14.13）。
