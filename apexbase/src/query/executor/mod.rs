...
impl ApexExecutor {
    ...
    pub fn execute_with_base_dir(
        ...
        Self::execute_classified_with_base_dir(sql, &sig, base_dir, default_table_path)
    }

    /// Execute a query whose signature was already computed by the caller.
    ///
    /// PyO3 and embedded entry points need the signature for locking and state
    /// management before invoking the executor. Passing it through keeps SQL
    /// classification single-pass without changing the public execution API.
    pub(crate) fn execute_classified_with_base_dir(
        sql: &str,
        ...
        if let Some(read) = sig.read_signature() {
            if let Some(result) =
                Self::try_execute_read_signature(read, base_dir, default_table_path)?
            {
                return Ok(result);
            }
        }
